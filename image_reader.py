...
def read_images_from_disk(input_queue, input_size, random_scale, random_mirror, ignore_label, img_mean): # optional pre-processing arguments
    ...
    #图像在存储时，并不是记录矩阵中的数字，而是记录经压缩编码之后的结果，要将图像还原成三维矩阵，需要对图像进行解码
    img = tf.image.decode_jpeg(img_contents, channels=3)
    img_r, img_g, img_b = tf.split(axis=2, num_or_size_splits=3, value=img)
    img = tf.cast(tf.concat(axis=2, values=[img_b, img_g, img_r]), dtype=tf.float32)  #RGB2BGR
    # Extract mean.
    img -= img_mean
...
class ImageReader(object):
    ...
    def __init__(self, data_dir, data_list, input_size,
        ...
        self.queue = tf.train.slice_input_producer([self.image_list, self.label_list],
